...
export class ProfileRepositoryClient implements ProfileService {
  async createProfile(
    profileData: Partial<Profile>
  ): Promise<ProfileResponseDTO> {
    try {
      const response = await axios.post<ApiResponse<ProfileResponseDTO>>(
  ...
  async readProfile(id: number): Promise<ProfileResponseDTO | null> {
    try {
      const response = await axios.get<ApiResponse<ProfileResponseDTO>>(
